...
from photo_album.auth import is_user_authorised, oauth
...
def format_caption(value):
    ...
    match = re.match(
        r"^(?P<date>\d{4}[-.]\d{2}[-.]\d{2})[_. ](?P<rest>.*)\.(?P<ext>[^.]+)$",
        filename,
    )
    if match:
        date_string = match.group("date").replace("-", ".")
        try:
            # a calendar date from a filename: no timezone applies
            formatted_date = date.fromisoformat(date_string.replace(".", "-")).strftime(
                "%d/%m/%Y"
            )
...
async def download(request: Request):
        ...
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
            for root, dirs, files in os.walk(settings.PHOTOS_DIR):
                for file in files:
                    ...
                    zipf.write(
                        file_path, os.path.relpath(file_path, settings.PHOTOS_DIR)
                    )
    ...
    except Exception as e:  # noqa: BLE001 - any failure must still clean up the temp file
...
async def auth(request: Request):
    ...
    try:
        if not hasattr(oauth, "google") or oauth.google is None:
            log.error("OAuth provider 'google' is not configured properly.")
            raise HTTPException(
                status_code=500, detail="OAuth provider not configured."
            )
